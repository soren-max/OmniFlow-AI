# API Design

## Current Stage

Currently implemented endpoints:

```
GET   /                            → Service info
GET   /health                      → Service health status
POST  /api/projects                → Create a content project
GET   /api/projects/{id}           → Get a content project by id
POST  /api/projects/{id}/preview   → Generate platform previews
POST  /api/projects/{id}/agent-preview → Run experimental LangGraph preview skeleton
POST  /api/projects/{id}/review/approve → Approve previews for mock publish
POST  /api/projects/{id}/review/reject  → Reject previews and block mock publish
POST  /api/projects/{id}/publish   → Mock publish selected platforms
POST  /api/projects/{id}/evaluation → Create a rule-based evaluation report
GET   /api/projects/{id}/evaluation → Get the latest rule-based evaluation report
GET   /api/runs/{run_id}           → Get an Agent Run trace record
GET   /api/runs/{run_id}/steps     → List Agent Step trace records for a run
GET   /docs                        → Swagger UI (auto-generated by FastAPI)
```

Projects, platform preview results, mock publish results, Agent Runs, and Agent
Steps are persisted in PostgreSQL through SQLAlchemy repositories and Alembic
migrations. Previews are generated using the PlatformAdapter mock adapters. Mock
publish is supported through `mock_publish`; no real platform API is called.
LangGraph is present only as a deterministic preview workflow skeleton and does
not call a real LLM.

Human Review is currently implemented as an API-level publish gate. Generating
previews moves a project to `pending`; reviewers can move it to `approved` or
`rejected`. Mock Publish requires `approved`. This is not yet a LangGraph
human-in-the-loop node.

Evaluation is currently implemented as deterministic rule-based quality analysis
for saved previews. It reports `format_score`, `style_score`,
`consistency_score`, `compliance_score`, `completeness_score`, `overall_score`,
`issues`, and `suggestions`. It does not call a real LLM and is not a substitute
for production content safety review. LLM-as-judge is planned for a future
evaluator.

Agent Run and Agent Step endpoints are read-only. They expose persisted trace
records written by the deterministic LangGraph `agent-preview` workflow. Trace
schemas, repository access, and lifecycle transitions are centralized in
`apps/api/app/telemetry/`; status values are `running`, `completed`, and `failed`.
The web demo can now show the returned `run_id` and load a basic Trace Viewer for
these records. They do not call real LLMs or publish to real platforms.

Supported preview platforms:

- `wechat` — WeChat Official Accounts / 微信公众号
- `zhihu` — Zhihu / 知乎
- `bilibili` — Bilibili / B站
- `xiaohongshu` — Xiaohongshu / 小红书
- `douyin` — Douyin / 抖音

## API Design Principles (Future)

The API will follow RESTful conventions:

- Use nouns for resources: `/projects`, `/contents`, `/runs`.
- Use HTTP methods for actions: GET, POST, PUT, DELETE.
- Use consistent pagination: `?page=1&size=20`.
- Use a consistent response envelope for core APIs.
- Use stable machine-readable error codes.
- Use Pydantic for request/response validation.
- Version via URL prefix if breaking changes are needed.

## Current Endpoints

### Projects

| Method | Path | Description |
|--------|------|-------------|
| POST | /api/projects | Create a new content project |
| GET | /api/projects/{id} | Get project details and saved previews |
| POST | /api/projects/{id}/preview | Generate previews for target platforms |
| POST | /api/projects/{id}/agent-preview | Run experimental deterministic LangGraph preview |
| POST | /api/projects/{id}/review/approve | Approve a project for mock publish |
| POST | /api/projects/{id}/review/reject | Reject a project and block mock publish |
| POST | /api/projects/{id}/publish | Mock publish selected target platforms |
| POST | /api/projects/{id}/evaluation | Create a rule-based quality evaluation report |
| GET | /api/projects/{id}/evaluation | Get the latest rule-based evaluation report |

### Agent Runs

| Method | Path | Description |
|--------|------|-------------|
| GET | /api/runs/{run_id} | Get one persisted Agent Run trace record |
| GET | /api/runs/{run_id}/steps | List persisted Agent Step records for a run |

Trace records are currently created by the deterministic LangGraph preview runner,
managed by the telemetry module, and stored in PostgreSQL.

### Evaluation — `POST /api/projects/{id}/evaluation`

Creates and persists a rule-based evaluation report for already generated
previews. The endpoint requires at least one saved preview.

```json
{
  "success": true,
  "data": {
    "project_id": "a1b2c3d4e5f6",
    "average_score": 86,
    "platform_scores": [
      {
        "platform": "wechat",
        "platform_display_name": "WeChat Official Accounts",
        "format_score": 95,
        "style_score": 84,
        "consistency_score": 88,
        "compliance_score": 100,
        "completeness_score": 80,
        "overall_score": 89,
        "issues": [],
        "suggestions": ["wechat: add platform-specific tags or hashtags"]
      }
    ],
    "issues": [],
    "suggestions": ["wechat: add platform-specific tags or hashtags"],
    "created_at": "2026-06-18T00:00:00Z"
  },
  "error": null
}
```

`GET /api/projects/{id}/evaluation` returns the latest saved report. If no report
exists yet, it returns `EVALUATION_NOT_FOUND`. If a project has no previews,
`POST /evaluation` returns `EVALUATION_REQUIRES_PREVIEW`.

### Create Project — `POST /api/projects`

```json
// Request
{
  "title": "My Article",
  "source_text": "Full article body...",
  "source_url": "https://example.com/original"  // optional
}

// Response (201 Created)
{
  "success": true,
  "data": {
    "id": "a1b2c3d4e5f6",
    "title": "My Article",
    "source_text": "Full article body...",
    "source_url": "https://example.com/original",
    "status": "created",
    "created_at": "2025-01-01T00:00:00Z",
    "previews": []
  },
  "error": null
}
```

### Mock Publish — `POST /api/projects/{id}/publish`

```json
// Request
{
  "target_platforms": ["wechat", "zhihu", "bilibili", "xiaohongshu", "douyin"],
  "mode": "mock"
}

// Response (200 OK)
{
  "success": true,
  "data": {
    "project_id": "a1b2c3d4e5f6",
    "mode": "mock",
    "results": [
      {
        "platform": "douyin",
        "platform_display_name": "Douyin",
        "status": "success",
        "mock_url": "mock://douyin/post/mock-123",
        "message": "Mock published to Douyin successfully.",
        "metadata": {
          "mock": true,
          "platform": "douyin"
        }
      }
    ],
    "published_at": "2025-01-01T00:00:00Z"
  },
  "error": null
}
```

Only `mode: "mock"` is supported. Mock Publish requires project status
`approved`; `pending` and `rejected` projects return `PROJECT_REVIEW_REQUIRED`.
`mode: "real"` returns `REAL_PUBLISH_NOT_SUPPORTED` because real publishing
requires explicit user authorization and secure platform credentials.

### Human Review — `POST /api/projects/{id}/review/approve`

Approves generated previews for Mock Publish and returns the updated project with
`status: "approved"`.

### Human Review — `POST /api/projects/{id}/review/reject`

Rejects generated previews and returns the updated project with
`status: "rejected"`. Rejected projects cannot be mock-published unless they are
approved again.

`GET /api/runs/{run_id}` returns an `AgentRun` record. `GET /api/runs/{run_id}/steps`
returns the ordered `AgentStep` records for that run. Both use the standard response
envelope. Regular Preview and Mock Publish do not write LangGraph step records yet;
they persist their own project preview and mock publish result rows.

### Get Project — `GET /api/projects/{id}`

```json
// Response (200 OK)
{
  "success": true,
  "data": {
    "id": "a1b2c3d4e5f6",
    "title": "My Article",
    "source_text": "Full article body...",
    "source_url": null,
    "status": "created",
    "created_at": "2025-01-01T00:00:00Z",
    "previews": []
  },
  "error": null
}
```

### Generate Preview — `POST /api/projects/{id}/preview`

```json
// Request
{
  "platforms": ["wechat", "zhihu", "bilibili", "xiaohongshu", "douyin"],
  "title": "Optional title override",  // optional
  "hooks": ["Did you know..."],        // optional
  "tags": ["tag1", "tag2"]             // optional
}

// Response (200 OK)
{
  "success": true,
  "data": {
    "project_id": "a1b2c3d4e5f6",
    "project_title": "My Article",
    "previews": [
      {
        "project_id": "a1b2c3d4e5f6",
        "platform": "douyin",
        "platform_display_name": "Douyin",
        "title": "My Article",
        "content": "<div class=\"douyin-video-script\">...</div>",
        "metadata": {
          "format": "douyin_short_video_script",
          "hook": "3秒看懂: My Article",
          "tags": ["AI工具", "内容运营"],
          "shot_list": ["镜头 1: 开场吸引, 直接抛出痛点或结果"],
          "call_to_action": "你更想看哪个平台的版本? 评论区告诉我。"
        },
        "preview": {
          "rendered_html": "<div>...preview HTML...</div>",
          "word_count": 1250,
          "estimated_read_time_min": 5,
          "screenshot_url": null,
          "metadata": {}
        },
        "validation": {
          "is_valid": true,
          "errors": [],
          "warnings": []
        },
        "rendered_html": "<div>...preview HTML...</div>",
        "word_count": 1250,
        "estimated_read_time_min": 5,
        "warnings": []
      }
    ],
    "generated_at": "2025-01-01T00:00:00Z"
  },
  "error": null
}
```

### Agent Preview — `POST /api/projects/{id}/agent-preview`

This experimental endpoint runs the deterministic LangGraph preview skeleton.
It returns workflow state with normalized input, platform strategy, previews,
errors, status, and `run_id`. It also creates one Agent Run trace and one Agent
Step trace per LangGraph node. It does not replace `/preview`, call real LLMs, or
perform real publishing. Platform previews are still generated through the
PlatformAdapter registry. The frontend uses this endpoint to expose demo-level
Agent Trace inspection after preview generation.

```json
// Response excerpt
{
  "success": true,
  "data": {
    "run_id": "f3c6e7...",
    "project_id": "a1b2c3d4e5f6",
    "status": "completed",
    "previews": {
      "wechat": { "...": "..." }
    }
  },
  "error": null
}
```

### Get Agent Run — `GET /api/runs/{run_id}`

```json
// Response (200 OK)
{
  "success": true,
  "data": {
    "run_id": "f3c6e7...",
    "project_id": "a1b2c3d4e5f6",
    "workflow_name": "content_preview",
    "status": "completed",
    "input_snapshot": {},
    "output_snapshot": {},
    "error_message": null,
    "started_at": "2025-01-01T00:00:00Z",
    "finished_at": "2025-01-01T00:00:01Z",
    "total_latency_ms": 1000,
    "created_at": "2025-01-01T00:00:00Z"
  },
  "error": null
}
```

### List Agent Steps — `GET /api/runs/{run_id}/steps`

```json
// Response (200 OK)
{
  "success": true,
  "data": [
    {
      "step_id": "b2f4...",
      "run_id": "f3c6e7...",
      "node_name": "intake",
      "status": "completed",
      "input_snapshot": {},
      "output_snapshot": {},
      "tool_calls": [],
      "error_message": null,
      "latency_ms": 12,
      "started_at": "2025-01-01T00:00:00Z",
      "finished_at": "2025-01-01T00:00:00Z",
      "created_at": "2025-01-01T00:00:00Z"
    }
  ],
  "error": null
}
```

## Intended Endpoints (Future)

### Projects

| Method | Path | Description |
|--------|------|-------------|
| GET | /api/v1/projects | List projects |
| POST | /api/v1/projects | Create project |
| GET | /api/v1/projects/{id} | Get project detail |
| PUT | /api/v1/projects/{id} | Update project |
| DELETE | /api/v1/projects/{id} | Delete project |

### Content

| Method | Path | Description |
|--------|------|-------------|
| POST | /api/v1/projects/{id}/contents | Submit content to Agent |
| GET | /api/v1/contents/{id} | Get content detail |
| GET | /api/v1/contents/{id}/previews | Get platform previews |

### Agent Runs

| Method | Path | Description |
|--------|------|-------------|
| GET | /api/v1/runs | List Agent runs |
| GET | /api/v1/runs/{id} | Get run detail |
| GET | /api/v1/runs/{id}/steps | Get run steps |
| POST | /api/v1/runs/{id}/approve | Approve for publish |
| POST | /api/v1/runs/{id}/reject | Reject content |

### Publishing

| Method | Path | Description |
|--------|------|-------------|
| POST | /api/v1/runs/{id}/publish | Publish to platforms |
| POST | /api/v1/runs/{id}/mock-publish | Mock publish for testing |

### Evaluations

| Method | Path | Description |
|--------|------|-------------|
| GET | /api/v1/runs/{id}/evaluation | Get evaluation report |

## Response Format

```json
{
  "success": true,
  "data": {},
  "error": null
}
```

## Error Format

```json
{
  "success": false,
  "data": null,
  "error": {
    "code": "PROJECT_NOT_FOUND",
    "message": "Project not found: a1b2c3d4e5f6",
    "details": {
      "project_id": "a1b2c3d4e5f6"
    }
  }
}
```

Current error codes include:

- `PROJECT_NOT_FOUND`
- `INVALID_PLATFORM`
- `ADAPTER_NOT_FOUND`
- `ADAPTER_EXECUTION_FAILED`
- `REAL_PUBLISH_NOT_SUPPORTED`
- `UNSUPPORTED_PUBLISH_MODE`
- `PROJECT_REVIEW_REQUIRED`
- `AGENT_RUN_NOT_FOUND`
- `VALIDATION_ERROR`
- `HTTP_ERROR`

## Currently Not Implemented

- **Real platform publishing** — only mock preview generation and mock publish results are implemented.
- **Project update/delete** — only create, read, preview, and mock publish exist.
- **Listing all projects** — no paginated list endpoint.
- **Authentication/authorization** — no user or API key system.
- **Full Human Review workflow** — API-level approve/reject gating exists, but
  graph-native human-in-the-loop review is not implemented yet.
- **Evaluation Report** — quality scoring and report persistence are not implemented yet.
